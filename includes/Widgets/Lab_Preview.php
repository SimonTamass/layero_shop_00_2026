...
class Lab_Preview extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_control('title', array(
			...
			'default' => 'Próbáld ki. <em>Most azonnal.</em>',
		));
		$this->add_control('text', array(
			'label' => __('Leírás', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXTAREA,
			'default' => 'Írd be a neved, és nézd meg, ahogy rétegről rétegre fénybe nyomtatjuk - pontosan így születik minden Layero darab.',
		));
	...
	protected function render() {
		...
		<section class="lyr-lab" data-layero-lab>
			<div class="lyr-lab__copy">
				...
				<h2><?php echo wp_kses($settings['title'] ?? '', array('em' => array())); ?></h2>
				<p><?php echo esc_html($settings['text'] ?? ''); ?></p>
				<form>
					<input type="text" maxlength="14" placeholder="<?php echo esc_attr__('pl. Dominik', 'layero-shop-ui'); ?>" aria-label="<?php echo esc_attr__('Név a nyomtatáshoz', 'layero-shop-ui'); ?>" autocomplete="off">
					<button class="lyr-btn lyr-btn--primary" type="submit"><?php echo esc_html__('Nyomtasd ki!', 'layero-shop-ui'); ?></button>
					<a class="lyr-link--light" href="<?php echo esc_url($settings['product_url']['url'] ?? '#'); ?>" data-layero-lab-link><?php echo esc_html__('Kérem lámpaként', 'layero-shop-ui'); ?> &rsaquo;</a>
				</form>
			</div>
